...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        hidden_layer_error = np.multiply(np.dot(self.hidden_to_output_weights.T, output_layer_error),
                                         np.vectorize(rectified_linear_unit_derivative)(hidden_layer_weighted_input))

        bias_gradients = hidden_layer_error
        hidden_to_output_weight_gradients = np.dot(output_layer_error, hidden_layer_activation.T)
        input_to_hidden_weight_gradients = np.dot(hidden_layer_error, input_values.T)

        # Use gradients to adjust weights and biases using gradient descent
        self.biases += bias_gradients * self.learning_rate
        self.input_to_hidden_weights += input_to_hidden_weight_gradients * self.learning_rate
        self.hidden_to_output_weights += hidden_to_output_weight_gradients * self.learning_rate
