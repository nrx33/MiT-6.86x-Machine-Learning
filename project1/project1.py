...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(n_samples):
            ...
            sum_theta += theta
            sum_theta_0 += theta_0
    avg_theta = sum_theta / (T * n_samples)
    avg_theta_0 = sum_theta_0 / (T * n_samples)
...
def classify(feature_matrix, theta, theta_0):
    ...
    # Convert predictions to 1s and -1s
    classifications = np.sign(predictions)
    
    # Handle the case when prediction is exactly 0
...
def bag_of_words(texts, remove_stopwords=False):
    """
