...
def compute_probabilities(X, theta, temp_parameter):
    ...
    scores = np.dot(theta, X.T) / temp_parameter
    
    # Stabilize the computation by subtracting the maximum score from each row
    c = np.max(scores, axis=0)
    stabilized_scores = scores - c
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    # Create a matrix where each column i represents the true class of the ith sample
    log_prob = np.log(probabilities)

    # Select the log probabilities of the correct classes
    correct_log_prob = log_prob[Y, np.arange(n)]

    # Compute the cross-entropy loss
    cross_entropy_loss = -np.sum(correct_log_prob) / n
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    # Create an indicator matrix for the labels
    indicator = sparse.coo_matrix((np.ones(n), (Y, np.arange(n))), shape=(k, n)).toarray()
    
    # Compute the gradient
    grad_theta = -np.dot(indicator - probabilities, X) / (temp_parameter * n)
    
    # Add the regularization term
    grad_theta += lambda_factor * theta
